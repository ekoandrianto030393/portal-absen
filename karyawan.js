...
module.exports = (pool) => {
    ...
    router.post('/register_face', async (req, res) => {
        ...
        try {
            ...
            // =================================================
            // 🔧 [FINAL] PERBAIKAN 2 — TOLERANSI JABATAN
            // =================================================
            ...
            const finalJabatan =
                jabatan && jabatan.trim() !== '' ? jabatan : 'Staff';
            // =================================================
    ...
    router.get('/:id', async (req, res) => {
        ...
        try {
            connection = await pool.getConnection();
            const [rows] = await connection.execute(
                'SELECT id_karyawan, nama, jabatan, foto FROM karyawan WHERE id_karyawan = ?',
                [req.params.id]
            );
            ...
            karyawan.foto = karyawan.foto
                ? `data:image/jpeg;base64,${Buffer.from(karyawan.foto).toString('base64')}`
                : null;
